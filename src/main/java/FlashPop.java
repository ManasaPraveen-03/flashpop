...
public class FlashPop extends JFrame {
    ...
    private List<Flashcard> quizQueue = new ArrayList<>();
    ...
    private void loadUsers() {
        ...
        if (file.exists()) {
            ...
            // create demo user if no file exists
    ...
    private JPanel createQuizPanel() {
        ...
        ActionListener nextQ = e -> {
            ...
            questionLabel.setText("<html><b>" + f.question + "</b></html>");
            scoreLabel.setText("Score: " + String.format("%.1f", f.getScore()) + "%");
        };

        knowBtn.addActionListener(e -> {
            Flashcard f = quizQueue.get(quizIndex);
            f.increaseScore();
            currentUser.knownCount++;
            saveUsers();
            quizIndex++;
        ...
        JPanel btnPanel = new JPanel(new FlowLayout());
        btnPanel.add(knowBtn);
        btnPanel.add(recapBtn);
    ...
    private JPanel createProgressPanel() {
        ...
        if (currentUser.flashcards.size() > 0) {
            double percent = (currentUser.knownCount * 100.0) / currentUser.flashcards.size();
            sb.append("Quiz Progress: ").append(String.format("%.2f", percent)).append("%\n");
        } else {
            sb.append("Quiz Progress: N/A\n");
        }
    ...
    static class Flashcard {
        ...
        public void increaseScore() { score = Math.min(100, score * 1.1); }
        public void decreaseScore() { score = Math.max(0, score * 0.9); }
    }
}
